...
class Camera:
    ...
    def toOrthogonalPoints(self, image, point):
        if self.perspectiveMatrix is None:
            self.initializeProjectionPoints(image)
        return cv2.perspectiveTransform(np.array([[point]], dtype=np.float32), self.perspectiveMatrix)
    ...
    def createSourceDestinationPointsFromChessBoard(filename, boardCornersShape):
        ...
        if found is True:
            ...
            return np.array([source[0][0], source[1][0], source[9][0], source[10][0]], np.float32), \
                   np.array([dest[0], dest[1], dest[9], dest[10]], np.float32)
    ...
    def getDefaultPerspectiveCorrectionPoints(self, forImage):
        ...
        # x_offset = W(1-1/l)*0.5
        x_offset = int(imageWidth * (1 - 1 / lanes) * 0.5)
...
# cam.calibrateFromImages('./camera_cal/calibration*.jpg', (9, 6), showResults=False)
# cam.saveCalibration('./calibration.p')
# cam.loadCalibration('calibration.p')
# undistorted, img = cam.undistortImageFromFile('calibration_wide/test_image.jpg')
