...
TEST_F(VehicleTest, returnsTheCorrectStateAtTime) {
  ...
  EXPECT_THAT(state,
              ElementsAre(2,
                          DoubleNear(124.8336 + 10 * 1 + 0.5 * 1 / 2, 0.1),
                          10.5,
                          0.5)); // lane , s, v, acc
  ...
  EXPECT_THAT(state,
              ElementsAre(2,
                          DoubleNear(124.8336 + 10 * 2 + 0.5 * 2 * 2 / 2, 0.1),
                          11.0,
                          0.5)); // lane , s, v, acc
}
...
TEST_F(VehicleTest, willCollideWithAtTime) {
  ...
  auto result =
      v1_.willCollideWith(v2_, 20, 1); // find if there is collision in 20*1 sec
  EXPECT_TRUE(result.first);
  EXPECT_EQ(result.second, 10);

  result = v1_.willCollideWith(v2_,
                               20,
                               0.1); // find if there is collision in 20*0.1 = 2 sec
  ...
  result = v1_.willCollideWith(v2_,
                               20,
                               1); // find if there is collision in 20*0.1 = 2 sec
  EXPECT_FALSE(result.first);
  EXPECT_EQ(result.second, -1);
}